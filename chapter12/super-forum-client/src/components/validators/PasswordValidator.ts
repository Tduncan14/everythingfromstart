...
export const isPasswordValid = (password:string): PasswordTestResult => {
   ...
   if(password.length < 8 ) {
    ...
    passwordTestResult.message="Password is must be at least 8 characters";
...
const strongPassword = new RegExp(
  ...
  if(!strongPassword.test(password)){
    ...
    passwordTestResult.message = "Password must contain at least 1 special charcter, 1 cap letter,and 1 number"
    passwordTestResult.isValid = false
  }
